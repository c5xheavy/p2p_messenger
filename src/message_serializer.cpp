...
std::vector<uint8_t> MessageSerializer::message_to_buffer(const Message& message, std::shared_ptr<dht::crypto::PublicKey> public_key) {
    ...
    std::size_t size{sizeof(message.id)
                     ...
                     + sizeof(std::uint16_t) + message.source_public_key.size()
    ...
    std::uint16_t source_public_key_size{static_cast<std::uint16_t>(message.source_public_key.size())};
...
std::vector<uint8_t> MessageSerializer::signed_message_to_buffer(const SignedMessage& signed_message) {
    std::size_t size{sizeof(std::uint16_t) + signed_message.message.size()
                     + sizeof(std::uint16_t) + signed_message.signature.size()};

    std::vector<uint8_t> buffer(size);
    uint8_t* ptr{buffer.data()};

    std::uint16_t message_size{static_cast<std::uint16_t>(signed_message.message.size())};
    ...
    std::uint16_t signature_size{static_cast<std::uint16_t>(signed_message.signature.size())};
